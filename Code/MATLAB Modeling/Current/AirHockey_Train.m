...
%AI VARIABLES
loadfile = 'genomes/airhockey_asdf.mat';
% ------ OR -------%
width     = 6;
height    = 1;
input     = 6;
...
%pop       = 200;
%elitism   = 0.1;
writefile = 'airhockey_learn4'; %without the .mat ending, leave blank for standard conv

try
   load(loadfile)
   G
catch
    G = TrainBrain(height,width,input,output,learn,writefile);
end


if mode == 1 || mode == 2
    env = RREnvTrain(puck,mallet,table,e,speed,mode,movie);
    env.brain = G;
    simulate(env,Inf)
elseif mode == 3
    c = 1;
