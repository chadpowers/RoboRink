classdef SimpleEnvTrain < handle
    ...
    methods
        ...
        % CONSTRUCTOR %
        function ENV = SimpleEnvTrain(puck,mallet,table,e,speed,mode,createMov)
        ...
        function setUp(ENV) 
            ...
            set (ENV.fig, 'KeyPressFcn',           @(src, eventdata)keyDownListener(ENV,src,eventdata));
            set (ENV.fig, 'KeyReleaseFcn',         @(src, eventdata)keyUpListener(ENV,src,eventdata));
            ...
            ENV.p1.obj = rectangle('Curvature',[1,1],'Position',[ENV.table.x/2 - ENV.p2.d/2,ENV.table.y - ENV.p2.d - 2,ENV.p2.d,ENV.p2.d],'FaceColor','b');
        ...
        function step(ENV,dt)
            ...
            if ENV.mode == 1
                ENV.p1.newp = [max(0,min(ENV.table.x-ENV.p1.d,ENV.mouse.x-ENV.p1.d/2)),...
                    max(0,min(ENV.table.y-ENV.p1.d,ENV.mouse.y-ENV.p1.d/2))];
            else
                ENV.p1.newp = [max(0+.2*ENV.puck.d,min(ENV.table.x-ENV.p1.d-.2*ENV.puck.d,ENV.puck.obj.Position(1)+ENV.puck.d/2-ENV.p2.d/2)-ENV.bias),...
            ...
            [b_inputs,b_outputs] = (TrainBrain.evaluateNet(ENV.brain.brain.gene,input,1));
            clc
            output = b_outputs(end).v
            
            if size(output,1) == 1
                ENV.p2.v = [40*(output-0.5),0];
            else
                ENV.p2.v = 40*[output(1)-0.5,output(2)-0.5];
            end
            newp = ENV.p2.obj.Position(1:2)+ENV.p2.v*dt;   
            ENV.p2.obj.Position(1:2) = newp;
        
            if ENV.p2.obj.Position(1) < 0
                ENV.p2.obj.Position(1) = 0;
                ENV.p2.v(1) = 0;
            elseif ENV.p2.obj.Position(1) > ENV.table.x - ENV.p1.d
                ENV.p2.obj.Position(1) = ENV.table.x - ENV.p1.d;
                ENV.p2.v(1) = 0;
            end
            if ENV.p2.obj.Position(2) < 0
                ENV.p2.obj.Position(2) = 0;
                ENV.p2.v(2) = 0;
            elseif ENV.p2.obj.Position(2) > ENV.table.y - ENV.p1.d
                ENV.p2.obj.Position(2) = ENV.table.y - ENV.p1.d;
                ENV.p2.v(2) = 0;
            end
        end
        
        function train(ENV)
            input = [[ENV.table.x,ENV.table.y]+ENV.puck.d-ENV.puck.obj.Position(1:2),...
                -ENV.puck.v,[ENV.table.x,ENV.table.y]+ENV.p1.d-ENV.p1.obj.Position(1:2)];
            output = -ENV.p1.v'/40+0.5;
            learn(ENV.brain,input,output)
            clc
            ENV.brain.brain.gene(end).m
        end
        ...
        function keyDownListener(ENV,object, eventdata)
            ENV.doTrain = 1;
        end
        
        function keyUpListener(ENV,object, eventdata)
            ENV.doTrain = 0;
        end
