classdef RREnv < handle
    ...
    methods
        ...
        function [g] = step(ENV,dt)
            ...
            ENV.p2.v = 20*(Genome.evaluateNet(ENV.brain.gene,input,.5)-0.5)';
            newp = ENV.p2.obj.Position(1:2)+ENV.p2.v*dt;
            if newp(1) < 0
                newp(1) = 0;
                ENV.p2.v(1) = 0;
                ENV.brain.fitness = ENV.brain.fitness - 0.1;
            ...
            elseif newp(2) > ENV.table.y-ENV.p2.d
                ...
                ENV.p2.v(2) = 0;
                ENV.brain.fitness = ENV.brain.fitness - 0.05;
            end
        ...
        function collision(ENV)
            ...
            if (y >= ENV.table.y - ENV.puck.d)
                ...
                ENV.brain.fitness = ENV.brain.fitness - 1/pdist([ENV.puck.obj.Position(1:2)+ENV.puck.d/2;ENV.table.x/2,ENV.table.y]);
            end
            ...
            if (y <=                0) 
                ...
                ENV.brain.fitness = ENV.brain.fitness + 5/pdist([ENV.puck.obj.Position(1:2)+ENV.puck.d/2;ENV.table.x/2,0]);
            end;
            ...
            if c
                ENV.brain.fitness = ENV.brain.fitness + 0.5/pdist([ENV.puck.obj.Position(1:2)+ENV.puck.d/2;ENV.table.x/2,0]);
            end
            [ENV.puck,c] = ENV.collisionwmallet(ENV.puck,ENV.p2,ENV.e);
            if c
                ENV.brain.fitness = ENV.brain.fitness - 4*ENV.puck.v(2);
            end
        end
